...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $query = mysql_query("SELECT $clause_select, SUM(tb_fakta.nilai_tiket) as z FROM $clause_from, tb_fakta WHERE $clause_where GROUP BY $group ORDER BY $group");

        $start_find = (strpos($group, ".")+1);
        if ($group == "tb_pelabuhan.id_pelabuhan") {
            $id = "id_pel_emb";
        }else{
            $id = substr($group, $start_find);
        }

        $result = mysql_query("SELECT $group as parameter, SUM(tb_fakta.nilai_tiket) as nilai_tiket FROM $check_group, tb_fakta WHERE $group=tb_fakta.$id GROUP BY $group ORDER BY nilai_tiket DESC LIMIT 10");
                    ...
                    dataPoints: 
                        [<?php 
                            ...
                            while($row = mysql_fetch_array($result)){
                                echo '{"label":"'.$row['parameter'].'", "y":'.$row['nilai_tiket'].'}';
                                if($i != $baris) {
                                    echo ",";
                                }
                                $i++;
